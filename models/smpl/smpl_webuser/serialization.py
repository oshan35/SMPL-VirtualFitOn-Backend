...
def save_model(model, fname):
    m0 = model
    trainer_dict = {
        'v_template': np.asarray(m0.v_template), 'J': np.asarray(m0.J),
        'weights': np.asarray(m0.weights), 'kintree_table': m0.kintree_table,
        'f': m0.f, 'bs_type': m0.bs_type, 'posedirs': np.asarray(m0.posedirs)
    }
    ...
    with open(fname, 'wb') as f:
        pickle.dump(trainer_dict, f, pickle.HIGHEST_PROTOCOL)
...
def load_model(fname_or_dict):
    ...
    args = {
        'pose': dd['pose'], 'v': dd['v_posed'], 'J': dd['J'], 'weights': dd['weights'],
        'kintree_table': dd['kintree_table'], 'xp': ch, 'want_Jtr': True, 'bs_style': dd['bs_style']
    }
