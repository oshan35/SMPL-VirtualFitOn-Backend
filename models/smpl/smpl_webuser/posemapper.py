...
class Rodrigues(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        """ Computes the derivative of the rotation matrix with respect to an input. """
        if wrt is self.rt:
            return cv2.Rodrigues(self.rt.r)[1].T


def lrotmin(p):
    """ Linearized rotation minimization function. """
...
def posemap(s):
    """ Maps a joint-angles to pose-blendshapes based on the mapping type. """
    if s == 'lrotmin':
        return lrotmin
    else:
        raise ValueError(f'Unknown posemapping: {s}')
